...
void app_main(void)
{
	...
	PARAMETER_t param_sta;
	sprintf(param_sta.ip, IPSTR, IP2STR(&ip_info_wifi.ip));
	param_sta.server_port = CONFIG_WIFI_SERVER_PORT;
	param_sta.ctrl_port = 32767;
	strcpy(param_sta.user_ctx, "WiFi");
	ESP_LOGD(TAG, "param_sta.ip=[%s]", param_sta.ip);
	xTaskCreate(&http_server, "HTTP_SERVER_STA", 1024*4, (void *)&param_sta, 5, NULL);
#endif
